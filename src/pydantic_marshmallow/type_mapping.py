...
def _get_simple_field_class(type_hint: type) -> type[ma_fields.Field[Any]]:
    """
...
def type_to_marshmallow_field(type_hint: Any) -> ma_fields.Field[Any]:
    """
    ...
    # Handle list[T]
    if origin is list:
        inner: ma_fields.Field[Any] = ma_fields.Raw()
        if args:
    ...
    # Handle dict[K, V]
    if origin is dict:
        key_field: ma_fields.Field[Any] = ma_fields.String()
        value_field: ma_fields.Field[Any] = ma_fields.Raw()
    ...
    if origin in (set, frozenset):
        inner_set: ma_fields.Field[Any] = ma_fields.Raw()
        if args:
